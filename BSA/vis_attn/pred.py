...
ROOT_DIR = Path(__file__).resolve().parents[1]  # 0是父目录，1是爷目录
...
from attn_src.fft_attn.my_attn_v6 import myattn_prefill, reset_adaptive_state, get_adaptive_state
...
def parse_args(args=None):
    ...
    parser.add_argument('--corr_selection_mode', type=str, default='threshold', choices=['threshold', 'topk'],
                        help='相关性 mask 选择模式：threshold 或 topk')
    ...
    parser.add_argument("--diag_sample_ratio", type=float, default=0.15,
                        help="条带判断时对角线（tau方向）的采样比例，相对于可用远程对角线总数")
    ...
    if key_states.shape[2] == query_states.shape[2]:
        ...
        if self.method == "myattn":
            ...
            attn_output = myattn_prefill(
                ...
                attention_vis_dir='./output/'+str(dataset),
            )
...
def get_pred(
    ...
    for idx, json_obj in enumerate(pbar):
        ...
        if seq_len >= 50_000:
            print("input 过长，截断至 50_000")
            input = tokenizer(prompt, return_tensors="pt", truncation=True, max_length=50_000).to("cuda")
            seq_len = input.input_ids.shape[-1]
...
def is_retrieval_task(model, tokenizer, dataset_name, prompt_format):
    ...
    print(
        f"📋 任务类型判断: {dataset_name} => {task_type} => {'检索类，调整参数' if result else '非检索类，保持原始参数'} (模型输出: '{generated}')")
    return result
...
if __name__ == "__main__":
    ...
    model2path = json.load(open(str(ROOT_DIR)+"/eval/LongBench/config/model2path.json", "r"))
    model2maxlen = json.load(open(str(ROOT_DIR)+"/eval/LongBench/config/model2maxlen.json", "r"))
    ...
    dataset2prompt = json.load(open(str(ROOT_DIR)+"/eval/LongBench/config/dataset2prompt.json", "r"))
    dataset2maxlen = json.load(open(str(ROOT_DIR)+"/eval/LongBench/config/dataset2maxlen.json", "r"))
    ...
    for dataset in datasets:
        ...
        if args.method == "full":
            ...
            if args.use_cor:
                ...
                param_str = ", ".join(map(str, param_list))
            out_path = f"eval/LongBench/pred/{model_name}/{dataset}-myattn_v6-[ {param_str} ].jsonl"
