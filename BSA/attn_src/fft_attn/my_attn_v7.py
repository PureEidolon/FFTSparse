...
def create_last_block_query_mask(
    ...
    scores = torch.einsum('hd,nhd->hn', Q_last, K_all)  # [H, N]

    k = min(max_blocks, N)
    _, topk_idx = torch.topk(scores, k=k, dim=-1)  # [H, k]

    mask = torch.zeros(H, N, dtype=torch.bool, device=Q_b.device)
    mask.scatter_(1, topk_idx, True)

    return mask.unsqueeze(0).unsqueeze(2)  # [1, H, 1, N]
...
def myattn_prefill(
    ...
    if enable_last_block_mask:
        last_block_mask = create_last_block_query_mask(Q_b, K_b)
    ...
    prior_kept = prior_mask.sum().item()
    prior_ratio = prior_kept / prior_mask.numel()

    corr_ratio = corr_mask.sum().item() / corr_mask.numel() if corr_mask is not None else 0.0
    qk_ratio = qk_mask.sum().item() / qk_mask.numel() if (
                enable_correlation_mask and q_block_num > correlation_top_k and num_diffuse > 0) else 0.0
    col_ratio = col_mask.sum().item() / col_mask.numel() if col_mask is not None else 0.0

    final_kept = final_mask.sum().item()
    final_ratio = final_kept / total

    print(
        f"[Layer {layer_idx}] prior={prior_ratio:.4f} | corr={corr_ratio:.4f} | qk={qk_ratio:.4f} | col={col_ratio:.4f} | final={final_ratio:.4f}")

    # 在 final_mask = final_mask.contiguous() 之前
    total = final_mask.numel()
    _layer_sparsity_stats[layer_idx] = {
        'prior': prior_mask.sum().item() / prior_mask.numel(),
        'corr': corr_mask.sum().item() / corr_mask.numel() if corr_mask is not None else 0.0,
        'qk': qk_mask.sum().item() / qk_mask.numel() if qk_mask is not None else 0.0,
        'col': col_mask.sum().item() / col_mask.numel() if col_mask is not None else 0.0,
        'final': final_mask.sum().item() / total,
    }
    ...
    if layer_idx == TOTAL_LAYERS - 1:
        print(f"\n{'=' * 70}")
        print(f"{'Layer':<8}{'Prior':<10}{'Corr':<10}{'QK':<10}{'Col':<10}{'Final':<10}")
        print('=' * 70)
        for i in range(TOTAL_LAYERS):
            if i in _layer_sparsity_stats:
                s = _layer_sparsity_stats[i]
                print(
                    f"{i:<8}{s['prior']:<10.4f}{s['corr']:<10.4f}{s['qk']:<10.4f}{s['col']:<10.4f}{s['final']:<10.4f}")
        # 平均值
        avg = {k: sum(s[k] for s in _layer_sparsity_stats.values()) / len(_layer_sparsity_stats)
               for k in ['prior', 'corr', 'qk', 'col', 'final']}
        print('-' * 70)
        print(
            f"{'AVG':<8}{avg['prior']:<10.4f}{avg['corr']:<10.4f}{avg['qk']:<10.4f}{avg['col']:<10.4f}{avg['final']:<10.4f}")
        print('=' * 70)
        _layer_sparsity_stats.clear()  # 清空，方便下次推理重新统计
