...
def xattn_estimate(
    ...
    # Triton 官方要求 SM >= 7.0，但 xattn 可能要求更高（如 8.0）
    # 保守起见：SM >= 7.5（支持 Tensor Core）即可尝试
    if use_triton and props.major < 7:
        use_triton = False
        print(
    ...
    if keep_recent:
        ...
        actual_head_num = simple_masks.shape[1]  # 动态获取
        eye_matrix_expanded = (
            eye_matrix.unsqueeze(0)
            .unsqueeze(0)
            .expand(1, actual_head_num, q_block_num, q_block_num)
        )
        simple_masks[:, :, -q_block_num:, -q_block_num:] = torch.where(
