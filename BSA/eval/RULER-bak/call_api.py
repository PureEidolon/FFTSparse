...
# RULER scripts 根目录
RULER_SCRIPTS = '/root/cjh/pro/resources/datasets/RULER/scripts'
...
parser.add_argument("--method", type=str, default="full",
                    help="attention method: full/myattn/xattn/flex/minference/sparge")
...
if args.server_type in ('hf', 'gemini', 'my_model'):
    args.threads = 1
...
def get_llm(tokens_to_generate):
    ...
    elif args.server_type == 'my_model':
        sys.path.insert(0, '/root/cjh/pro/BSA/eval/RULER')
        from my_model_wrapper import MyCustomModel
        llm = MyCustomModel(
            name_or_path=args.model_name_or_path,
            method=args.method,
            attn_cfg=dict(
                ...
                enable_last_block=args.enable_last_block,
                last_block_thres=args.last_block_thres,
            ),
            max_new_tokens=tokens_to_generate,
            stop=args.stop_words,
        )
...
def main():
    ...
    # 把 RULER scripts 加入 sys.path，以便找到 data.synthetic.constants
    if RULER_SCRIPTS not in sys.path:
        sys.path.insert(0, RULER_SCRIPTS)

    module = importlib.import_module(f"data.{args.benchmark}.constants")

    tasks_base = module.TASKS
    with open(os.path.join(RULER_SCRIPTS, f"{args.benchmark}.yaml"), "r") as f:
