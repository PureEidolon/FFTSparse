...
def find_blocks_chunked(
    ...
    if threshold is not None:
        ...
        if isinstance(threshold, torch.Tensor):
            ...
            # 如果 threshold 的 head 数和实际不符，截断或填充
            if threshold.shape[0] != head_num:
                if threshold.shape[0] > head_num:
                    threshold = threshold[:head_num]  # 截断，如32→28
                else:
                    threshold = torch.cat([  # 填充，如28→32
                        threshold,
                        threshold[-1].unsqueeze(0).expand(head_num - threshold.shape[0])
                    ])
