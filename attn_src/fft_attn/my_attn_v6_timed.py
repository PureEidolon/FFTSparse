...
def batch_fft_cross_correlation(Q_b, K_b, local_window=0, keep_sink=0):
    ...
    Q_t = Q_b.permute(1, 2, 0).contiguous().float()  # [H, D, N]
    K_t = K_b.permute(1, 2, 0).contiguous().float()
    fft_size = 2 * N
    Q_fft = torch.fft.rfft(Q_t, n=fft_size, dim=-1)  # [H, D, fft_size//2+1]
    ...
    corr_total = corr_per_dim.sum(dim=1)  # [H, fft_size]
...
def compute_stripe_variance(scores, remote_start, num_sample_diags=5):
    ...
    # 用最短对角线长度统一截断
    min_diag_len = N - max(sample_taus) - 1
    if min_diag_len < 2:
        mean_var = torch.full((H,), float('inf'), device=device)
        return mean_var, torch.zeros(H, T, device=device)

    # 构造行列索引
    row_start = max(sample_taus)
    row_idx = torch.arange(row_start, row_start + min_diag_len, device=device)  # [L]
    col_idx = row_idx.unsqueeze(0) - taus.unsqueeze(1)  # [T, L]

    # batch gather [H, T, L]
    H_idx = torch.arange(H, device=device).view(H, 1, 1).expand(H, T, min_diag_len)
    T_row = row_idx.unsqueeze(0).unsqueeze(0).expand(H, T, min_diag_len)
    T_col = col_idx.unsqueeze(0).expand(H, T, min_diag_len)
    diag_batch = scores[H_idx, T_row, T_col]  # [H, T, L]

    # 一次性算差分方差
    diff = diag_batch[:, :, 1:] - diag_batch[:, :, :-1]  # [H, T, L-1]
    diff_var = diff.var(dim=-1)  # [H, T]
    diag_mean = diag_batch.abs().mean(dim=-1).clamp(min=1e-6)  # [H, T]
