...
def compute_stripe_variance(
    ...
    # 可视化
    if is_visual:
        is_stripe_for_vis = mean_var <= stripe_threshold  # 仅供可视化，不影响外部逻辑
...
def myattn_prefill(
    ...
    # ========== 阶段 1.5: padding 到 block_size 整数倍 ==========
    ...
    L_q_padded = L_q + pad_len_q
    L_k_padded = L_k + pad_len_k
    ...
    # ===== 根据上一层的触发状态决定当前层参数 =====
    # retrieval_triggered 是上一层或更早触发的，layer_idx > trigger_layer 才生效
    ...
    if enable_correlation_mask and q_block_num > correlation_top_k:
        ...
        if not _adaptive_state['retrieval_triggered']  and  layer_idx <= 10:
            ...
            # 调试打印
            print(f"  [Layer {layer_idx}] 检索检测: "
                  f"high_var_heads={high_var_per_head.sum().item()}/{H}, "
                  f"high_mean_heads={high_mean_per_head.sum().item()}/{H}, "
                  f"combined_ratio={combined_ratio:.2f}, "
                  f"mean stripe_variance={stripe_variance.mean().item():.3f}, "
                  f"max per_tau_var={per_tau_var.max().item():.3f}")
        ...
        # 统一在这里判断 is_stripe，用 effective_stripe_threshold
    ...
    attn_output = attn_output[:, :, :L_q, :]  # 裁掉 padding
